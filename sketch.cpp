...
{   /**in default, w = 51, k = 51, is_hpc = 1**/\
    ...
    /* Reads with no annotation entry are left unfiltered (vanilla behavior). */\
    if (ha_anno_active && ha_anno && p->n > 0) {\
        int64_t arid = (int64_t)p->a[0].rid;\
        if (arid >= 0 && arid < ha_anno->n_reads && ha_anno->reads[arid].n > 0) {\
            int32_t sn = 0;\
            for (i = 0; i < (int)p->n; ++i) {\
